...
SYNC_CACHE_KEY = "last_global_sync_champions"
SYNC_LOCK_KEY = "sync_in_progress_champions"
...
class FootballDataAPI:
    ...
    def get_competition_matches(self, competition="CL", status=None, stage=None):
        params = {}
        if status:
    ...
    def sync_active_matches(self, force=False):
        ...
        try:
            all_matches = self.get_competition_matches(competition="CL")
            synced_count = 0
    ...
    def sync_matches_to_db(self, batch_size=10, delay_between_batches=BATCH_DELAY):
        from .models import Match
        matches = self.get_competition_matches(competition="CL")
